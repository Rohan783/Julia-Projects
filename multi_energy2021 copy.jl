...
function equation(dpol_dt,pol::Matrix{Z},v,T) where {Z <: Number}
    ...
    L_e =  take_norm(L_0 .+ (3/8)*(2*pol[19,:] .+ pol[20,:]).*Lscale)
    L_mu = take_norm(L_0 .+ (3/8)*(2*pol[20,:] .+ pol[19,:]).*Lscale)
    ...
    dpol_dt[19,:] .= take_norm(((dpol_dt[5,:]-dpol_dt[13,:]) + ((dpol_dt[10,:]-dpol_dt[18,:])*sqrt3))/(2*Lscale))
    dpol_dt[20,:] .= take_norm(((dpol_dt[13,:]-dpol_dt[5,:]) + ((dpol_dt[10,:]-dpol_dt[18,:])*sqrt3))/(2*Lscale))
    ...
    dpol_dt[3:4,:] += 0.5*G2T5[[CartesianIndex()],:] .*(0.85*pol[3:4,:] .+ 0.75*pol[11:12,:])
    dpol_dt[6:7,:] += 0.5*G2T5[[CartesianIndex()],:] .*(3.56*pol[6:7,:])
    dpol_dt[8:9,:] += 0.5*G2T5[[CartesianIndex()],:] .*(2.5*pol[8:9,:])

    dpol_dt[11:12,:] += 0.5*G2T5[[CartesianIndex()],:] .*(0.85*pol[11:12,:] .+ 0.75*pol[3:4,:])
    dpol_dt[14:15,:] += 0.5*G2T5[[CartesianIndex()],:] .*(3.56*pol[14:15,:])
    dpol_dt[16:17,:] += 0.5*G2T5[[CartesianIndex()],:] .*(2.5*pol[16:17,:])
...
sol = solve(prob, TRBDF2(),maxiters=1e6,tstops=np.linspace(60e6,1e6,10000))#,abstol=1e-16,reltol=1e-16)
