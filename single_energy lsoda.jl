...
theta_all = np.array(asin.(sqrt.([3*10^-3,3*10^-2,6*10^-4]))/2)
...
function GMcross(pot_all,pol,dpol_dt)
    # R_k = V_i*P_j*F_ijk
    sq3 = sqrt(3)
    #add +1 
    # Remember that V_i = V[:,i-1]
...
vac_for_all_x = transpose(vac_coeff[[CartesianIndex()],:] ./ (2*x))
...
function equation(dpol_dt,pol,v,T) #where {Z <: Number}
    T=(100e6 - T)/10^6
    # T=T/10^6
    ...
    L_e = (pol[19,:]).*Lscale
    L_mu = (pol[20,:]).*Lscale
    chem_p_e = -3.628*pi*sinh.(asin.(-1.2087*(pol[19,:] .*Lscale))/3)
    chem_p_mu= -3.628*pi*sinh.(asin.(-1.2087*(pol[20,:] .*Lscale))/3)
    ...
    #println(size(dpol_dt))
    dpol_dt[3:18,:] = GMcross(2*v,pol,dpol_dt)
...
#pol_ini =  repeat([4/3,4/3,0,0,0,0,0,0,0,(2/sqrt(3)),0,0,0,0,0,0,0,(2/sqrt(3)),Asym_e/Lscale,Asym_mu/Lscale],lenx)

tspan = (60.0*1e6,1*1e6)
tspan_ = (40e6, 50e6)

prob = ODEProblem(equation,pol_ini,tspan_,v)
# sol = solve(prob, RadauIIA5(),maxiters=1e6,abstol=1e-18,reltol=1e-18)
sol = solve(prob, lsoda(),maxiters=1e7,abstol=1e-16,reltol=1e-16)
# sol = solve(prob, TaylorMethod(8),abstol=1e-18)#,reltol=1e-18)
#tv, xv = taylorinteg(equation, pol_ini, tspan[1], tspan[2], 28, 1e-20, maxsteps=10000);
#TRBDF2
...
#plot(sol)
for i=1:lenx
    plt.plot(sol.t,1 .- 0.5*(sol[1,i,:]+ sol[5,i,:] + (sol[10,i,:]/sqrt(3))))
    plt.plot(sol.t,1 .- 0.5*(sol[2,i,:]+ sol[13,i,:] + (sol[18,i,:]/sqrt(3))))
end
